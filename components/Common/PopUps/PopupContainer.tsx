...
const PopupContainer = ({ children, ...props }: PopupContainerProps) => {
    ...
    const setClose = () => {
        ...
        tl.to('#popup_container', { duration: 0.6, opacity: 0 }, 'b')
            .to('#popup_inner', { duration: 0.6, y: '100vh', opacity: 0, onComplete: () => props.setShowPopup(false) }, 'b')
    }

    return (
        <div onClick={setClose} id='popup_container' className='fixed h-screen w-screen left-0 top-0 flex items-center justify-center overflow-y-scroll bg-gray-200/50 z-[999] opacity-0 px-3 py-4'>
